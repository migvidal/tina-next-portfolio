...
export default function NavItems({ navs }: { navs: any }) {
  ...
  return (
    <ul className="flex gap-6 sm:gap-8 lg:gap-10 tracking-[.002em] -mx-4 flex-1 justify-center">
      {navs.map((item) => {
        return (
          <li
            key={item.href}
          ...
          >
            <Link
              ...
              href={`/${item.href}`}
              className={`relative select-none text-base font-semibold inline-block tracking-wide transition duration-150 ease-out hover:opacity-100 py-4`}
            >
